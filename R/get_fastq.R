...
#' @param get_sra_file logical, whether to download SRA file first and get fastq files afterwards or directly download fastq files.
#' @param sra_files_dir directory where SRA files are saved. If you use \code{get_sra_file=FALSE} then \code{sra_files_dir=NULL}.
...
get_fastq <- function(srr_id, library_layout=c("SINGLE","PAIRED"), get_sra_file=FALSE, sra_files_dir=NULL, n_thread, destdir) {
	...
	#system(paste0("parallel-fastq-dump --outdir . --threads ",n_thread, " --skip-technical  --readids --read-filter pass --dumpbase --split-files --clip --sra-id ",
	#sra_files_dir,"/",srr_id,".sra"))
